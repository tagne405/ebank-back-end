...
public class EBanqueApplication {
    ...
    CommandLineRunner Start(BankAccountService bankAccountService){
        return args -> {
            ...
            bankAccountService.listCustomer().forEach(customer -> {
                try {
                    bankAccountService.saveCurrentBankAccount(Math.random()*2121212,500, customer.getId());
                    bankAccountService.saveSavingBankAccount(Math.random()*30000,2.2, customer.getId());
                    List<BankAccountDto> bankAccounts = bankAccountService.bankAccountsList();
